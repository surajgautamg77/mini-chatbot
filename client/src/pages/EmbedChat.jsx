...
function EmbedChat() {
  // The useChat hook already handles picking up chatbotId and sessionId from URL params
  const { messages, loading, sendMessage, clearHistory, chatbotId } = useChat();
  ...
  const handleSend = async () => {
    if (!inputValue.trim() || loading || !chatbotId) return;
  ...
  if (!chatbotId) {
    return <div className="flex items-center justify-center h-screen text-gray-400 text-xs italic">Initialization failed: No Bot ID</div>;
  }
    ...
    <div className="flex flex-col h-screen bg-white font-sans overflow-hidden border border-gray-100 shadow-2xl">
      ...
      <div className="p-4 bg-primary-600 text-white flex justify-between items-center shadow-md">
        <div className="flex items-center space-x-2">
          ...
          <div>
            <h1 className="font-bold text-sm tracking-wide">Assistant</h1>
        ...
        <div className="flex items-center space-x-3">
          <button 
             onClick={() => { if(confirm('Clear conversation?')) clearHistory(); }}
      ...
      {/* Messages */}
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50/50 text-[13px]">
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center opacity-40 px-6">
            ...
            <p className="text-[11px] leading-relaxed">How can I help you with our documents today?</p>
          </div>
        ) : (
          messages.map((m) => (
      ...
      <div className="p-4 bg-white border-t border-gray-50">
        <div className="flex items-center bg-gray-50 border border-[#dfe1e5] rounded-full px-4 py-0.5 focus-within:bg-white focus-within:shadow-md transition-all duration-200">
          <input
            type="text"
            value={inputValue}
            ...
            className="flex-1 bg-transparent border-none focus:ring-0 text-sm py-2.5 text-gray-800"
            disabled={loading}
          />
          <button
            onClick={handleSend}
            disabled={!inputValue.trim() || loading}
            className={`${inputValue.trim() ? 'text-primary-600' : 'text-gray-300'} transition-all hover:scale-110`}
          >
